...
const WorkflowEditor = () => {
  ...
  useEffect(() => {
    const fetchWorkflow = async () => {
      ...
      try {
        const response = await authenticatedFetch(workflowUrl(workspaceId, workflowId));
        if (response.ok) {
  ...
  if (errorMessage) {
    return (
      <div className="flex flex-col items-center justify-center h-screen gap-3 bg-surface dark:bg-surface-dark" role="alert">
        <h1 className="text-lg font-semibold text-text-primary dark:text-text-primary-dark">Unable to load workflow</h1>
        <p className="text-sm text-text-secondary dark:text-text-secondary-dark">{errorMessage}</p>
      </div>
    );
  }
