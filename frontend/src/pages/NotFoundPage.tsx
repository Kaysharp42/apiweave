...
export function NotFoundPage() {
  ...
  const contextPath =
    orgSlug && workspaceSlug ? `/${orgSlug}/${workspaceSlug}` : "";

  return (
    <div className="relative flex min-h-[60vh] overflow-hidden bg-surface p-10 dark:bg-surface-dark lg:p-14">
      <svg
        className="pointer-events-none absolute inset-0 h-full w-full text-border opacity-40 dark:text-border-dark"
        aria-hidden="true"
      >
      ...
      <div className="relative z-10 flex max-w-3xl flex-col justify-center gap-6">
        ...
        <div>
          <Button
            ...
            size="sm"
            onClick={() =>
              navigate(contextPath ? `${contextPath}/workflows` : "/")
            }
          >
            Back to workflows
          </Button>
