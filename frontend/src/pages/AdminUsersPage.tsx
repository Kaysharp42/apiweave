...
export default function AdminUsersPage() {
  ...
  const [state, dispatch] = useReducer(
    (current: AdminUsersState, action: AdminUsersAction): AdminUsersState => {
      switch (action.type) {
        case "set-users":
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      dispatch({
        type: "set-invites",
        value: inviteData.filter(
          (inv) => !inv.consumed && new Date(inv.expires_at) > now,
        ),
      });
    } catch {
  ...
  const handleRoleChange = async (userId: string, newRole: string) => {
    if (userId === currentUser?.userId && newRole !== "admin") {
      const adminCount = state.users.filter((u) =>
    ...
    try {
      ...
      dispatch({
        type: "set-users",
        value: state.users.map((u) => (u.userId === userId ? updatedUser : u)),
      });
      setSelectedUser((current) =>
        current?.type === "user" && current.user.userId === userId
  ...
  const handleInviteRoleChange = async (inviteId: string, newRole: string) => {
    try {
      await authenticatedJson(`${API_BASE_URL}/api/invites/${inviteId}/role`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ role_preset: newRole }),
      });
      const updatedInvites = state.invites.map((inv) =>
        inv.inviteId === inviteId ? { ...inv, role_preset: newRole } : inv,
      );
      dispatch({ type: "set-invites", value: updatedInvites });
      setSelectedUser((current) => {
        if (
          current?.type === "invite" &&
          current.invite.inviteId === inviteId
        ) {
          return {
            type: "invite",
            invite: { ...current.invite, role_preset: newRole },
          };
        }
        if (
          current?.type === "user" &&
          current.pendingInvite?.inviteId === inviteId
        ) {
          return {
            ...current,
            pendingInvite: { ...current.pendingInvite, role_preset: newRole },
          };
        }
        return current;
      });
      ...
      const msg =
        err instanceof Error ? err.message : "Failed to update invite role";
      toast.error(msg);
    }
  };
  ...
  const handleCopyInviteLink = async (inviteId: string, inviteUrl: string) => {
    ...
    setTimeout(
      () => dispatch({ type: "set-copying-invite-id", value: null }),
      1500,
    );
  };
    ...
    try {
      if (state.deleteConfirm.type === "user") {
        await authenticatedFetch(
          `${API_BASE_URL}/api/users/${state.deleteConfirm.id}`,
          {
            method: "DELETE",
          },
        );
        dispatch({
          ...
          value: state.users.filter(
            (u) => u.userId !== state.deleteConfirm?.id,
          ),
        });
        setSelectedUser((current) =>
          current?.type === "user" &&
          current.user.userId === state.deleteConfirm?.id
            ? null
            : current,
        );
        ...
        await authenticatedFetch(
          `${API_BASE_URL}/api/invites/${state.deleteConfirm.id}`,
          {
            method: "DELETE",
          },
        );
        dispatch({
          type: "set-invites",
          value: state.invites.filter(
            (inv) => inv.inviteId !== state.deleteConfirm?.id,
          ),
        });
        setSelectedUser((current) => {
          if (
            current?.type === "invite" &&
            current.invite.inviteId === state.deleteConfirm?.id
          )
            return null;
          if (
            current?.type === "user" &&
            current.pendingInvite?.inviteId === state.deleteConfirm?.id
          ) {
  ...
  const inviteByEmail = new Map(
    state.invites.map((inv) => [inv.email.toLowerCase(), inv]),
  );
  ...
  const orphanInvites = state.invites.filter(
    (inv) =>
      !state.users.some(
        (u) => u.verified_email.toLowerCase() === inv.email.toLowerCase(),
      ),
  );
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="flex justify-end">
              <Button
                ...
                onClick={() =>
                  dispatch({ type: "set-invite-modal-open", value: true })
                }
              >
                Invite User
              </Button>
                        ...
                        <>
                          {state.users.map((user) => {
                            ...
                            const pendingInvite = !user.is_setup_complete
                              ? inviteByEmail.get(
                                  user.verified_email.toLowerCase(),
                                )
                              : undefined;
                            ...
                            const isSelected =
                              selectedUser?.type === "user" &&
                              selectedUser.user.userId === user.userId;
                          ...
                          {orphanInvites.map((inv) => {
                            const isSelected =
                              selectedUser?.type === "invite" &&
                              selectedUser.invite.inviteId === inv.inviteId;
                            return (
                              <tr
                                key={inv.inviteId}
                                className={`border-b border-border dark:border-border-dark last:border-0 hover:bg-surface-overlay dark:hover:bg-surface-dark-overlay transition-colors focus-within:outline-2 focus-within:outline-[var(--aw-primary)] focus-within:outline-offset-[-2px] cursor-pointer ${isSelected ? "bg-primary/5 dark:bg-primary-light/10" : ""}`}
                                ...
                                <td className="px-6 py-4">
                                  <div className="flex items-center gap-2">
                                    ...
                                    {inv.invite_url && (
                                      ...
                                      >
                                        {state.copyingInviteId === inv.inviteId
                                          ? "Copied!"
                                          : "Copy Link"}
                                      </Button>
                                    )}
