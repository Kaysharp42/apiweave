...
test("AdminUsersPage includes InviteUserModal and invite button", () => {
  ...
  assert.ok(content.includes("Invite User"), "Must have an Invite User button");
});
...
test("AdminUsersPage shows role selector with admin/editor/viewer options", () => {
  ...
  assert.ok(
    ...
    "Must include admin role option",
  );
  assert.ok(
    content.includes('value="editor"'),
    "Must include editor role option",
  );
  assert.ok(
    content.includes('value="viewer"'),
    "Must include viewer role option",
  );
});
...
test("AdminUsersPage shows user status badges (Active/Pending)", () => {
  ...
  assert.ok(
    ...
    "Must show Active status for setup-complete users",
  );
  assert.ok(
    content.includes("Pending"),
    "Must show Pending status for incomplete users",
  );
  assert.ok(
...
test("AdminUsersPage refreshes user list after invite modal closes", () => {
  const content = readPage("AdminUsersPage.tsx");
  assert.ok(content.includes("fetchUsers"), "Must define fetchUsers callback");
...
test("InviteUserModal shows invite URL after successful creation", () => {
  const content = readAuthComponent("InviteUserModal.tsx");
  assert.ok(content.includes("inviteUrl"), "Must track invite URL state");
  assert.ok(
...
test("AdminUsersPage derives Active status from is_setup_complete=true", () => {
  const content = readPage("AdminUsersPage.tsx");
  assert.ok(
    (content.includes("is_setup_complete") && content.includes('"Active"')) ||
      content.includes("'Active'") ||
      content.includes('label="Active"'),
    "Must show Active label when is_setup_complete is true",
  );
  assert.ok(
    content.includes('"Pending"') ||
      content.includes("'Pending'") ||
      content.includes('label="Pending"'),
    "Must show Pending label when is_setup_complete is false",
  );
  assert.ok(
    content.includes('"Invited"') ||
      content.includes("'Invited'") ||
      content.includes('label="Invited"'),
...
test("AdminUsersPage prevents self-delete by hiding delete button for current user", () => {
  ...
  assert.ok(
    content.includes("currentUser?.userId") ||
      content.includes("currentUser?.userId"),
