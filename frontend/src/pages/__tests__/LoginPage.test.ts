...
function assertIncludes(markup: string, expected: string): void {
  assert.ok(
    ...
    `Expected content to include ${expected}: ${markup}`,
  );
}
...
function startProviderVisibilityFlow(): {
  ...
  const run = (async () => {
    try {
      ...
      state.providers = getEnabledProviders(data).map(
...
function assertProviderPageSource(
  fileName: string,
  emptyMessage: string,
): void {
...
function assertOAuthHookPageSource(
  fileName: string,
  emptyMessage: string,
): void {
  const content = readPage(fileName);
...
test("LoginPage shows loading state while provider request is pending", async () => {
  ...
  try {
    ...
    deferred.resolve(
      new Response(
        JSON.stringify([{ id: "github", enabled: true, label: "GitHub" }]),
        {
          status: 200,
          headers: { "Content-Type": "application/json" },
        },
...
test("SetupPage source keeps provider visibility UI and loading/error states", () => {
  assertProviderPageSource("SetupPage.tsx", "No sign-in providers configured");
});

test("InvitePage source keeps provider visibility UI and loading/error states", () => {
  assertProviderPageSource("InvitePage.tsx", "No sign-in providers configured");
});
