...
const SCOPE_CONFIG: Record<string, { variant: 'primary' | 'secondary' | 'success' | 'info'; label: string }> = {
...
export function ScopeBadge({ scopeType, className = '' }: ScopeBadgeProps) {
  const config = SCOPE_CONFIG[scopeType] ?? { variant: 'primary' as const, label: scopeType };
