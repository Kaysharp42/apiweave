...
function makeTokenResponse(overrides: Partial<ServiceTokenCreateResponse> = {}): ServiceTokenCreateResponse {
  return {
    tokenId: 'tok-1',
    name: 'CI/CD Token',
...
describe('ServiceTokenCreateForm', () => {
  ...
  function renderForm(props: Partial<React.ComponentProps<typeof ServiceTokenCreateForm>> = {}) {
    return render(
  ...
  it('calls API and invokes onCreated with token response', async () => {
    ...
    await user.type(screen.getByPlaceholderText('CI/CD Deploy Token'), 'My Token');
  ...
  it('clears form fields after successful creation', async () => {
    ...
    const nameInput = screen.getByPlaceholderText('CI/CD Deploy Token') as HTMLInputElement;
    expect(nameInput.value).toBe('');
  });

  it('CRITICAL: token value is passed to onCreated exactly once', async () => {
    const user = userEvent.setup();
    const tokenResponse = makeTokenResponse({ token: 'one-time-only-token-value' });
    mockAuthenticatedJson.mockResolvedValueOnce(tokenResponse);
    ...
    const callArgs = mockOnCreated.mock.calls[0]![0] as ServiceTokenCreateResponse;
