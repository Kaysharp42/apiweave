...
const ACTOR_OPTIONS = ['', 'user', 'org_app', 'service_token', 'mcp_client', 'webhook_token', 'system_migration'] as const;
...
function withField<K extends keyof AuditEventFilter>(
  ...
  const next = { ...base };
  if (value) {
    (next as Record<K, AuditEventFilter[K]>)[key] = value as AuditEventFilter[K];
  } else {
    delete next[key];
  }
  return next;
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
      <div>
        ...
        >
          {ACTOR_OPTIONS.map((opt) => (
            <option key={opt} value={opt}>{opt || 'All actors'}</option>
          ))}
        </select>
      </div>
      ...
      <div>
        <Input
          ...
          onChange={(e) => onChange(withField(filters, 'action', (e.target as HTMLInputElement).value))}
        />
      </div>
      ...
      <div>
        ...
        >
          {SCOPE_OPTIONS.map((opt) => (
            <option key={opt} value={opt}>{opt || 'All scopes'}</option>
          ))}
        </select>
      </div>
      ...
      <div>
        <Input
          ...
          onChange={(e) => onChange(withField(filters, 'resourceType', (e.target as HTMLInputElement).value))}
        />
      </div>
      ...
      <div>
        <Input
          ...
          type="date"
          value={filters.from ?? ''}
          onChange={(e) => onChange(withField(filters, 'from', (e.target as HTMLInputElement).value))}
        />
      </div>
      ...
      <div>
        <Input
          ...
          type="date"
          value={filters.to ?? ''}
          onChange={(e) => onChange(withField(filters, 'to', (e.target as HTMLInputElement).value))}
        />
      </div>
    </div>
  );
