...
const intentClassName: Record<NonNullable<IconSwitchProps['intent']>, string> = {
  primary: 'bg-primary/10 dark:bg-primary-light/10 border-primary dark:border-primary-light text-primary dark:text-primary-light',
  success: 'bg-status-success/10 dark:bg-[var(--aw-status-success)]/10 border-status-success dark:border-[var(--aw-status-success)] text-status-success dark:text-[var(--aw-status-success)]',
};
  ...
  return (
    <button
      ...
      className={[
        ...
        checked ? intentClassName[intent] : 'bg-surface-raised dark:bg-surface-dark-raised border-border dark:border-border-dark text-text-muted dark:text-text-muted-dark',
        disabled ? 'cursor-not-allowed opacity-50 pointer-events-none' : 'cursor-pointer hover:bg-surface-overlay dark:hover:bg-surface-dark-overlay',
        className,
      ].filter(Boolean).join(' ')}
      {...rest}
    >
      ...
      >
        <span className={checked ? 'text-primary dark:text-primary-light' : 'text-text-muted dark:text-text-muted-dark'}>
