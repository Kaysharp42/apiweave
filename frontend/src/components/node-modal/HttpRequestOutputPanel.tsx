...
function statusVariant(statusCode: number | undefined): NonNullable<BadgeProps['variant']> {
    ...
    return (
      <div className="flex h-full min-h-0 items-center justify-center bg-surface p-6 dark:bg-surface-dark">
        <div className="text-center">
          ...
          <p className="mb-2 text-sm text-text-muted dark:text-text-muted-dark">Execute this node to view data</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-shrink-0 flex-col gap-3 border-b border-border bg-surface-raised px-4 py-3 dark:border-border-dark dark:bg-surface-dark-raised">
        <div className="flex min-w-0 flex-wrap items-center justify-between gap-2">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            <Badge variant={statusVariant(statusCode)} size="sm">{statusCode || '—'}</Badge>
            {durationLabel && <Badge variant="outline" size="sm"><Timer className="h-3 w-3" />{durationLabel}</Badge>}
            <Badge variant="outline" size="sm">{responseSize}</Badge>
            <span className="min-w-0 truncate font-mono text-xs text-text-secondary dark:text-text-secondary-dark">{requestLabel}</span>
          </div>
          <div className="flex items-center gap-1">
            <IconButton tooltip={copyLabel === 'curl' ? 'Copied cURL' : 'Copy as cURL'} size="sm" variant="secondary" onClick={() => void copyCommand('curl')}>
              <Copy className="h-4 w-4" />
            </IconButton>
            <IconButton tooltip={copyLabel === 'fetch' ? 'Copied fetch' : 'Copy as fetch'} size="sm" variant="secondary" onClick={() => void copyCommand('fetch')}>
              <Code2 className="h-4 w-4" />
            </IconButton>
          </div>
        </div>
        <SearchInput value={filterQuery} onChange={setFilterQuery} placeholder="Search response keys or values" size="sm" />
      </div>
