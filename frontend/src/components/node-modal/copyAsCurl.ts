...
import { normalizeHttpRequestConfig, parseKeyValuePairs } from './httpRequestConfigCompat';

function shellQuote(value: string): string {
  return `'${value.replace(/'/g, `'"'"'`)}'`;
}

function activePairs(value: NodeModalHTTPRequestConfig['headers']): KeyValuePair[] {
...
function authHeaders(config: NodeModalHTTPRequestConfig): KeyValuePair[] {
  ...
  if (auth.type === 'basic' && auth.basic) return [{ key: 'Authorization', value: `Basic ${auth.basic.username}:${auth.basic.password}` }];
  if (auth.type === 'apiKey' && auth.apiKey?.addTo === 'header') return [{ key: auth.apiKey.key, value: auth.apiKey.value }];
  return [];
}

function authQueryParams(config: NodeModalHTTPRequestConfig): KeyValuePair[] {
  const auth = config.auth;
  if (auth?.type === 'apiKey' && auth.apiKey?.addTo === 'query') return [{ key: auth.apiKey.key, value: auth.apiKey.value }];
  return [];
}

function bodyData(config: NodeModalHTTPRequestConfig): string | undefined {
  if (config.bodyType === 'none' || config.bodyType === 'binary') return undefined;
...
export function buildCurlCommand(rawConfig: NodeModalHTTPRequestConfig): string {
  const config = normalizeHttpRequestConfig(rawConfig);
  const queryParams = [...activePairs(config.queryParams), ...authQueryParams(config)];
  const url = appendQueryParams(config.url || 'https://api.example.com', queryParams);
  ...
  headers.forEach((header) => parts.push('--header', shellQuote(`${header.key}: ${header.value}`)));
...
export function buildFetchCommand(rawConfig: NodeModalHTTPRequestConfig): string {
  const config = normalizeHttpRequestConfig(rawConfig);
  const queryParams = [...activePairs(config.queryParams), ...authQueryParams(config)];
  const url = appendQueryParams(config.url || 'https://api.example.com', queryParams);
  const headers = Object.fromEntries([...activePairs(config.headers), ...authHeaders(config)].map((pair) => [pair.key, pair.value]));
  const body = bodyData(config);
