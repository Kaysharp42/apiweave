...
export function NodeModalVerticalTabs({ tabs, activeTab, onTabChange }: NodeModalVerticalTabsProps) {
  return (
    <div className="flex w-36 flex-shrink-0 flex-col border-r border-border bg-surface-overlay py-2 dark:border-border-dark dark:bg-surface-dark-overlay max-xl:w-14" role="tablist" aria-label="HTTP request sections">
      {tabs.map((tab) => {
        const Icon = tab.icon;
        ...
        return (
          <Tippy key={tab.key} content={tab.label} placement="right" delay={[300, 0]}>
            {tabButton}
          </Tippy>
        );
      })}
...
export function NodeModalResponsePane({ children, title = 'Response', onHide }: NodeModalResponsePaneProps) {
  return (
    <div className="flex h-full min-h-0 flex-col bg-surface dark:bg-surface-dark">
      <div className="flex flex-shrink-0 items-center justify-between border-b border-border bg-surface-overlay px-4 py-2 dark:border-border-dark dark:bg-surface-dark-overlay">
        <div className="flex items-center gap-2 font-mono text-xs font-semibold uppercase tracking-wide text-text-primary dark:text-text-primary-dark">
          <Eye className="h-4 w-4 text-text-muted dark:text-text-muted-dark" aria-hidden="true" />
          {title}
        </div>
        {onHide && (
          <IconButton tooltip="Hide response" size="sm" variant="ghost" onClick={onHide}>
    ...
    >
      <div className="flex h-full min-h-0 flex-col bg-surface-raised dark:bg-surface-dark-raised">
        <div className="flex flex-shrink-0 items-center gap-3 border-b border-border px-4 py-3 dark:border-border-dark">
          ...
          <Badge variant="secondary" size="sm" className="capitalize">{typeLabel}</Badge>
          <IconButton tooltip="Close" size="sm" variant="ghost" onClick={onClose}>
            <X className="h-5 w-5" />
          </IconButton>
        </div>
        ...
        <div className="flex min-h-0 flex-1">
          <NodeModalVerticalTabs tabs={tabs} activeTab={activeTab} onTabChange={onTabChange} />
          <div className="relative min-w-0 flex-1 bg-surface dark:bg-surface-dark">
            <Allotment className="h-full">
              <Allotment.Pane minSize={360} preferredSize={responseCollapsed ? '100%' : '58%'}>
                <div className={`h-full min-h-0 overflow-y-auto p-4 ${responseCollapsed ? 'pr-12' : ''}`}>{children}</div>
              </Allotment.Pane>
              {!responseCollapsed && (
                <Allotment.Pane minSize={340} preferredSize="42%">
                  <NodeModalResponsePane onHide={() => setResponseCollapsed(true)}>{responsePane}</NodeModalResponsePane>
                </Allotment.Pane>
              )}
            </Allotment>
            {responseCollapsed && (
