...
function isTreeOutput(output: unknown): output is Record<string, unknown> | unknown[] {
...
function normalizeStatus(status: string): StatusBadgeProps['status'] {
  if (status === 'running' || status === 'success' || status === 'error' || status === 'warning' || status === 'info') return status;
  return 'idle';
}
...
export function NodeOutputPanel({ output, executionStatus = 'idle' }: NodeOutputPanelProps) {
  ...
  const showTree = activeTab === 'tree' && !isEmptyOutput(output) && isTreeOutput(output);
    ...
    <div className="flex h-full min-h-0 flex-col bg-surface dark:bg-surface-dark">
      <div className="flex flex-shrink-0 flex-wrap items-center justify-between gap-2 border-b border-border bg-surface-raised px-4 py-3 dark:border-border-dark dark:bg-surface-dark-raised">
        <div className="flex min-w-0 items-center gap-2">
          <FileText className="h-4 w-4 text-text-muted dark:text-text-muted-dark" aria-hidden="true" />
          <h3 className="font-mono text-xs font-semibold uppercase tracking-wide text-text-primary dark:text-text-primary-dark">Node Output</h3>
        </div>
      ...
      {isEmptyOutput(output) ? (
        <div className="flex min-h-0 flex-1 items-center justify-center bg-surface p-6 dark:bg-surface-dark">
          <EmptyState
            icon={<FileText className="h-12 w-12 text-text-muted dark:text-text-muted-dark" strokeWidth={1.5} />}
            title="No output yet"
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden bg-surface p-4 dark:bg-surface-dark">
          <div className="flex flex-wrap gap-2">
            <Button variant={activeTab === 'tree' ? 'primary' : 'secondary'} size="xs" onClick={() => setActiveTab('tree')} disabled={!isTreeOutput(output)}>
              <Braces className="h-3.5 w-3.5" aria-hidden="true" />
              Tree
            </Button>
            <Button variant={activeTab === 'raw' ? 'primary' : 'secondary'} size="xs" onClick={() => setActiveTab('raw')}>
          ...
          {showTree ? (
            <Card title="Output tree" icon={BracesCardIcon} className="flex min-h-0 flex-1 flex-col [&>:last-child]:min-h-0 [&>:last-child]:flex-1">
            ...
            <Card title="Raw output" icon={FileCodeCardIcon} className="flex min-h-0 flex-1 flex-col [&>:last-child]:min-h-0 [&>:last-child]:flex-1">
