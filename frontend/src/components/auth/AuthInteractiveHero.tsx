...
import { CheckCircle2, Circle, Play, Square, type LucideIcon } from 'lucide-react';
...
const NODES: FourNodes = [
  ...
  { pos: { x: 350, y: 70 }, label: 'HTTP Request', sub: 'GET /users', Icon: Circle, result: '200' },
  { pos: { x: 90, y: 190 }, label: 'Assertion', sub: 'status == 200', Icon: CheckCircle2, result: '✓' },
  ...
  return (
    ...
    >
      ...
      <m.div
        ...
        style={{ top: 'clamp(6px,0.6vw,12px)', right: 'clamp(6px,0.6vw,12px)', padding: 'clamp(2px,0.25vw,5px) clamp(5px,0.5vw,9px)' }}
      ...
      {/* Edges */}
      <svg viewBox={`0 0 ${CARD_W} ${CARD_H}`} preserveAspectRatio="none" className="absolute inset-0 w-full h-full pointer-events-none">
        <line x1={NODES[0].pos.x} y1={NODES[0].pos.y} x2={NODES[1].pos.x} y2={NODES[1].pos.y} stroke={step === 1 ? 'var(--aw-primary)' : 'var(--aw-border)'} strokeWidth={1.5} vectorEffect="non-scaling-stroke" strokeDasharray={step === 1 ? '5 5' : undefined} />
        <line x1={NODES[1].pos.x} y1={NODES[1].pos.y} x2={NODES[2].pos.x} y2={NODES[2].pos.y} stroke={step === 2 ? 'var(--aw-primary)' : 'var(--aw-border)'} strokeWidth={1.5} vectorEffect="non-scaling-stroke" strokeDasharray={step === 2 ? '5 5' : undefined} />
        <line x1={NODES[2].pos.x} y1={NODES[2].pos.y} x2={NODES[3].pos.x} y2={NODES[3].pos.y} stroke={step === 3 ? 'var(--aw-primary)' : 'var(--aw-border)'} strokeWidth={1.5} vectorEffect="non-scaling-stroke" strokeDasharray={step === 3 ? '5 5' : undefined} />
      </svg>
      ...
      <m.div
        ...
        style={{
          ...
          boxShadow: '0 0 0 calc(clamp(7px,0.7vw,13px) * 0.45) var(--aw-primary)',
        }}
        animate={{
          left: pxX(pulsePos.x),
          top: pxY(pulsePos.y),
      ...
      {NODES.map((node, i) => {
        ...
        return (
          <div key={i} className="absolute" style={{ left: pxX(node.pos.x), top: pxY(node.pos.y), transform: 'translate(-50%, -50%)' }}>
            <div
              className={`flex items-center bg-surface-raised dark:bg-surface-dark-raised border rounded-sm cursor-default ${
                lit ? 'border-primary ring-2 ring-primary' : 'border-border dark:border-border-dark'
              }`}
              style={{
                width: NODE_W,
            ...
            >
              <node.Icon
                ...
                strokeWidth={2}
                style={{ width: 'clamp(11px,1vw,17px)', height: 'clamp(11px,1vw,17px)' }}
              />
              <span className="flex flex-col min-w-0">
                <span className="font-semibold text-text-primary dark:text-text-primary-dark truncate leading-tight" style={{ fontSize: 'clamp(9px,0.8vw,14px)' }}>
                  {node.label}
                </span>
                {node.sub && (
                  <span className="font-mono text-text-secondary dark:text-text-secondary-dark truncate leading-tight" style={{ fontSize: 'clamp(8px,0.65vw,11px)' }}>
                    {node.sub}
                  </span>
                )}
              </span>
              {node.result && (
                <m.span
                  ...
                  style={{ fontSize: 'clamp(8px,0.65vw,11px)', padding: 'clamp(1px,0.15vw,3px) clamp(3px,0.3vw,6px)' }}
...
export function AuthInteractiveHero() {
  ...
  return (
    <div data-testid="auth-hero" className="w-full h-full relative overflow-hidden" aria-hidden="true">
      {/* Static (reduced-motion) version — a completed run snapshot */}
      <div data-testid="auth-hero-static" className="absolute inset-0 hidden motion-reduce:flex flex-col gap-6 p-8 md:p-10 lg:p-14">
      ...
      <LazyMotion features={domAnimation}>
        <div data-testid="auth-hero-animated" className="absolute inset-0 flex motion-reduce:hidden flex-col gap-6 p-8 md:p-10 lg:p-14">
          <m.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="flex flex-col gap-2">
            <h2 className="font-display font-extrabold tracking-tight text-text-primary dark:text-text-primary-dark text-[clamp(1.75rem,4vw,3.25rem)] leading-[0.95]">
              APIWeave
            </h2>
            <p className="font-mono text-sm text-text-secondary dark:text-text-secondary-dark">
          ...
          <m.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: 'easeOut', delay: 0.15 }} className="flex-1 min-h-0 flex items-center justify-center">
...
function RunDiagramStatic() {
  return (
    <div className="relative w-full border border-border dark:border-border-dark bg-surface-raised dark:bg-surface-dark-raised rounded-sm overflow-hidden" style={CARD_STYLE}>
      <div className="absolute inset-0 opacity-[0.05] dark:opacity-[0.07] bg-[linear-gradient(currentColor_1px,transparent_1px),linear-gradient(90deg,currentColor_1px,transparent_1px)] bg-[size:clamp(16px,1.6vw,28px)_clamp(10px,1vw,18px)] text-text-muted dark:text-text-muted-dark" />
      <div aria-hidden="true" className="absolute inset-0 opacity-[0.06] dark:opacity-[0.09] pointer-events-none mix-blend-multiply dark:mix-blend-screen" style={{ backgroundImage: NOISE_DATA_URI, backgroundSize: '240px 240px' }} />
      ...
      <svg viewBox={`0 0 ${CARD_W} ${CARD_H}`} preserveAspectRatio="none" className="absolute inset-0 w-full h-full pointer-events-none">
        <line x1={NODES[0].pos.x} y1={NODES[0].pos.y} x2={NODES[1].pos.x} y2={NODES[1].pos.y} stroke="var(--aw-border)" strokeWidth={1.5} vectorEffect="non-scaling-stroke" />
        <line x1={NODES[1].pos.x} y1={NODES[1].pos.y} x2={NODES[2].pos.x} y2={NODES[2].pos.y} stroke="var(--aw-border)" strokeWidth={1.5} vectorEffect="non-scaling-stroke" />
        <line x1={NODES[2].pos.x} y1={NODES[2].pos.y} x2={NODES[3].pos.x} y2={NODES[3].pos.y} stroke="var(--aw-border)" strokeWidth={1.5} vectorEffect="non-scaling-stroke" />
      </svg>
      {NODES.map((node, i) => (
        <div key={i} className="absolute" style={{ left: pxX(node.pos.x), top: pxY(node.pos.y), transform: 'translate(-50%, -50%)' }}>
          <div className="flex items-center bg-surface-raised dark:bg-surface-dark-raised border border-primary ring-2 ring-primary rounded-sm cursor-default" style={{ width: NODE_W, gap: 'clamp(4px,0.4vw,8px)', padding: 'clamp(5px,0.5vw,9px) clamp(7px,0.65vw,12px)' }}>
            <node.Icon className="shrink-0 text-primary dark:text-primary-light" strokeWidth={2} style={{ width: 'clamp(11px,1vw,17px)', height: 'clamp(11px,1vw,17px)' }} />
            <span className="flex flex-col min-w-0">
              <span className="font-semibold text-text-primary dark:text-text-primary-dark truncate leading-tight" style={{ fontSize: 'clamp(9px,0.8vw,14px)' }}>{node.label}</span>
