...
test("AuthInteractiveHero contains reduced-motion fallback", () => {
  const content = readFileSync(
    join(AUTH_DIR, "AuthInteractiveHero.tsx"),
    "utf-8",
  );
  assert.ok(
  ...
  assert.ok(
    content.includes("motion-reduce:flex") ||
      content.includes("motion-reduce:block"),
...
test("LoginPage fetches configured-only providers and preserves inline states", () => {
  ...
  assert.ok(
    content.includes("useOAuthProviders"),
...
test("InvitePage reads token and shows providers", () => {
  ...
  assert.ok(
    content.includes("useParams<{ token: string }>()"),
    "Should extract token from params",
  );
  assert.ok(
    content.includes("login(providerId, token)"),
    "Should pass token to login",
  );
});
