...
const WEBHOOK_URL_PLACEHOLDER =
  "$APIWEAVE_BASE_URL/api/webhooks/workflows/WEBHOOK_ID/execute";
...
function buildJenkinsSnippet(mode: SnippetMode, auth: SnippetAuth): string {
  const url =
    "${env.APIWEAVE_BASE_URL}/api/webhooks/workflows/WEBHOOK_ID/execute";
  if (mode === "fire-and-forget") {
    if (auth === "token-only") {
      return `pipeline {
  agent any
  ...
  const auths: { id: SnippetAuth; label: string; desc: string }[] = [
    {
      id: "token-only",
      label: "Token Only",
      desc: "Development only when HMAC is not required",
    },
    {
      id: "hmac",
      label: "HMAC",
      desc: "Required in production with timestamp signing",
    },
  ];
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-text-muted dark:text-text-muted-dark">
        Copy a ready-to-use snippet into your CI/CD pipeline. Snippets use
        environment variable placeholders -- never actual secret values.
      </p>
