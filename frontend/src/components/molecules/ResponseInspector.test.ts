...
const documentStub: Record<string, unknown> = {
  ...
  getElementsByTagName: (tagName: string) => (tagName === 'head' ? [documentStub.head] : []),
};
...
const originalDispatcherDescriptor = Object.getOwnPropertyDescriptor(reactCurrentDispatcher, 'current');
...
const installForcedTabDispatcher = (): (() => void) => {
  ...
  Object.defineProperty(reactCurrentDispatcher, 'current', {
    ...
    set: (nextDispatcher: ReactDispatcher | null) => {
      ...
      const originalDispatcherUseState = nextDispatcher.useState.bind(nextDispatcher);
      currentDispatcher = {
        ...nextDispatcher,
        useState<State>(initialState: State | (() => State)): [State, Dispatch<SetStateAction<State>>] {
          useStateCallCount += 1;
  ...
  return () => {
    if (originalDispatcherDescriptor) {
      Object.defineProperty(reactCurrentDispatcher, 'current', originalDispatcherDescriptor);
...
const renderInspector = (
  ...
  useStateCallCount = 0;
  const restoreDispatcher = options.activeTab ? installForcedTabDispatcher() : () => undefined;
...
const createMetadata = (overrides: Partial<NodeResultMetadata> = {}): NodeResultMetadata => ({
...
test('ResponseInspector renders all six response tabs', () => {
  ...
  ['Tree', 'Raw', 'Headers', 'Cookies', 'Preview', 'Timing'].forEach((label) => {
...
test('ResponseInspector renders response summary header with status, content-type, and response-time chip', () => {
  const markup = renderInspector(
    createResponse({ status: 201, headers: { 'content-type': 'application/json; charset=utf-8' }, responseTime: 88 }),
...
test('ResponseInspector preview tab renders HTML in a sandboxed iframe', () => {
  const htmlBody = '<main><h1>Preview</h1><script>window.parent.hacked = true</script></main>';
  const markup = renderInspector(
    createResponse({
...
test('ResponseInspector timing tab displays metadata timing and response size accurately', () => {
  const markup = renderInspector(
    ...
    {
      activeTab: 'timing',
      metadata: createMetadata({ responseTimeMs: 123, responseSizeBytes: 1536 }),
    },
  );
...
test('ResponseInspector chooses the default tab from Content-Type and body format', () => {
  const jsonMarkup = renderInspector(createResponse({ headers: { 'content-type': 'application/vnd.api+json' } }));
  ...
  const metadataMarkup = renderInspector(
    ...
    { metadata: createMetadata({ contentType: 'text/html', bodyFormat: 'html' }) },
  );
...
test('ResponseInspector renders legacy response shapes without metadata', () => {
  const legacyResponse = createResponse({
    headers: {
      ...
      'Set-Cookie': 'sid=abc; Secure; HttpOnly; Path=/, theme=dark; SameSite=Lax',
    },
    body: 'legacy body',
    responseTime: 17,
  });

  const headersMarkup = renderInspector(legacyResponse, { activeTab: 'headers' });
  assertIncludes(headersMarkup, 'Headers (2)');
  assertIncludes(headersMarkup, 'Content-Type');
  assertIncludes(headersMarkup, 'text/plain');

  const cookiesMarkup = renderInspector(legacyResponse, { activeTab: 'cookies' });
...
test('ResponseInspector renders structured persisted cookies without Set-Cookie headers', () => {
  ...
  const cookiesMarkup = renderInspector(structuredResponse, { activeTab: 'cookies' });
