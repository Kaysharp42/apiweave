...
describe('Panel', () => {
  ...
  it('renders headerActions in the header', () => {
    ...
    expect(within(header as HTMLElement).getByRole('button', { name: 'Action' })).toBeInTheDocument();
  });
...
describe('PanelTabs', () => {
  ...
  it('marks the active tab with aria-selected=true', () => {
    ...
    expect(screen.getByRole('tab', { name: 'Config' })).toHaveAttribute('aria-selected', 'false');
  });

  it('invokes onTabChange with the new tab key when clicked', async () => {
    const onTabChange = vi.fn();
    const user = userEvent.setup();
    render(<PanelTabs tabs={tabs} activeTab="config" onTabChange={onTabChange} />);
...
describe('ConfirmDialog', () => {
  ...
  it('renders the default title when not provided', () => {
    render(
      <ConfirmDialog open onClose={() => {}} onConfirm={() => {}} message="msg" />,
    );
...
describe('PromptDialog', () => {
  ...
  it('renders a dialog with a text input when open', () => {
    render(
      <PromptDialog open onClose={() => {}} onSubmit={() => {}} />,
    );
    const dialog = screen.getByRole('dialog');
...
describe('SearchInput', () => {
  it('renders a textbox with the placeholder', () => {
    render(<SearchInput value="" onChange={() => {}} placeholder="Search workflows" />);
...
describe('KeyValueEditor', () => {
  ...
  it('invokes onChange with the updated key when a key input changes', () => {
    ...
    fireEvent.change(screen.getByLabelText('Key 1'), { target: { value: 'baz' } });
  ...
  it('invokes onChange with the updated value when a value input changes', () => {
    ...
    const pairs = [{ key: 'foo', value: 'bar' }];
    render(<KeyValueEditor pairs={pairs} onChange={onChange} />);
    fireEvent.change(screen.getByLabelText('Value 1'), { target: { value: 'qux' } });
...
describe('EmptyState', () => {
  it('renders the title and description', () => {
    ...
    expect(screen.getByText('Invite people to your organization.')).toBeInTheDocument();
  });
...
describe('WorkspaceEmptyState', () => {
  it('renders the welcome heading and description', () => {
    ...
    expect(screen.getByRole('heading', { name: /Welcome to APIWeave/i })).toBeInTheDocument();
    expect(screen.getByText(/Build visual API test flows/i)).toBeInTheDocument();
  });
  ...
  it('hides buttons whose handler is not provided', () => {
    ...
    expect(screen.queryByRole('button', { name: /Import Workflow/i })).toBeNull();
    expect(screen.queryByRole('button', { name: /Open Collection/i })).toBeNull();
  });
});
...
describe('StatusBadge', () => {
  ...
  it('uses the default label for each status', () => {
    const cases: Array<['idle' | 'running' | 'success' | 'error' | 'warning' | 'info', string]> = [
      ['idle', 'Idle'],
      ['running', 'Running'],
      ['success', 'Success'],
      ['error', 'Failed'],
