...
describe('WebhookManager', () => {
  beforeEach(() => {
    ...
    (authenticatedFetch as ReturnType<typeof vi.fn>).mockImplementation((url: string) => {
      ...
      if (url.includes('/api/webhooks/')) {
        return Promise.resolve({ ok: true, json: () => Promise.resolve([mockWebhook]) });
      }
      return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
    });
  });
  ...
  it('shows runId in toast after successful test delivery', async () => {
    ...
    (authenticatedFetch as ReturnType<typeof vi.fn>).mockImplementation((url: string, options?: RequestInit) => {
