...
describe('Task 13: Personal workspace boot — no legacy URLs', () => {
  ...
  it('must NOT fetch from legacy flat API routes during /personal/workflows boot', async () => {
    ...
    const legacyMatches = capturedUrls.filter((url) => LEGACY_URL_PATTERN.test(url));
  ...
  it('all boot fetches must use scoped workspace URLs', async () => {
    ...
    for (const url of capturedUrls) {
      if (url.includes('/api/workspaces/') || url.includes('/api/scopes/') || url.includes('/api/orgs') || url.includes('/api/users/')) {
        continue;
      }
      if (url.includes('/api/me') || url.includes('/api/auth') || url.includes('/api/csrf')) {
        continue;
      }
      expect(
        url,
        `Boot fetch URL does not use scoped pattern: ${url}`,
      ).toMatch(/\/api\/(workspaces|scopes|orgs|users|me|auth|csrf)/);
    }
  });
});
