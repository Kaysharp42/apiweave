...
describe('scopedApi', () => {
  describe('workflowsUrl', () => {
    it('builds a workspace-scoped workflow URL with default pagination', () => {
      ...
      expect(url).toBe(`${API_BASE_URL}/api/workspaces/ws-personal/workflows?skip=0&limit=20`);
    });

    it('accepts custom skip and limit', () => {
      const url = workflowsUrl('ws-org-42', { skip: 10, limit: 5 });
      expect(url).toBe(`${API_BASE_URL}/api/workspaces/ws-org-42/workflows?skip=10&limit=5`);
    });
  ...
  describe('workflowUrl', () => {
    it('builds a workspace-scoped single workflow URL', () => {
      ...
      expect(url).toBe(`${API_BASE_URL}/api/workspaces/ws-personal/workflows/wf-1`);
    });
  });
  ...
  describe('environmentsUrl', () => {
    ...
    it('builds a URL for organization-scoped environments', () => {
      const url = environmentsUrl({ scopeType: 'organization', scopeId: 'org-42' });
    ...
    it('builds a URL for all accessible workspace environments', () => {
      ...
      expect(url).toBe(`${API_BASE_URL}/api/workspaces/ws-1/environments/all-accessible?org_id=org-1`);
    });
  });
  ...
  describe('projectsUrl', () => {
    ...
    it('builds a scoped project export URL', () => {
      ...
      expect(url).toBe(`${API_BASE_URL}/api/workspaces/ws-1/projects/proj-99/export?include_environment=true`);
    });

    it('builds scoped project import URLs', () => {
      expect(projectImportUrl('ws-1')).toBe(`${API_BASE_URL}/api/workspaces/ws-1/projects/import`);
      expect(projectImportUrl('ws-1', true)).toBe(`${API_BASE_URL}/api/workspaces/ws-1/projects/import/dry-run`);
    });
  });
