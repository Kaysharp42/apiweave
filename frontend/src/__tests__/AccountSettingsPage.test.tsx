...
describe("AccountSettingsPage", () => {
  ...
  describe("local user", () => {
    it('shows "Local account" label and change-password section', () => {
      ...
      expect(screen.getAllByText("admin@local.test")).toHaveLength(2);
    ...
    it("does NOT show a sign-out button for local users", () => {
      ...
      expect(screen.queryByText(/Sign out of/)).not.toBeInTheDocument();
    });
  });
  ...
  describe("OAuth user (github)", () => {
    it('shows provider label "Continue with GitHub" and sign-out button', () => {
      ...
      expect(screen.getAllByText("dev@github.test")).toHaveLength(2);
      expect(
  ...
  describe("OAuth user (google)", () => {
    it('shows provider label "Continue with Google"', () => {
      ...
      expect(screen.getAllByText("user@google.test")).toHaveLength(2);
    });
  });
  ...
  describe("sign-out button", () => {
    it("calls signOut when clicked", async () => {
      ...
      const signOutButton = screen.getByText(
        "Sign out of Continue with GitHub",
      );
