...
test("buildSwaggerRefreshSummary formats success message with definition count", () => {
  const summary = buildSwaggerRefreshSummary(
    { definitionCount: 3, failedDefinitionCount: 0 },
    124,
  );

  assert.equal(
    summary.successMessage,
    "Swagger refreshed: 124 endpoints from 3 definitions.",
  );
...
test("buildSwaggerRefreshSummary includes warning for partial failures", () => {
  const summary = buildSwaggerRefreshSummary(
    { definitionCount: 4, failedDefinitionCount: 1 },
    88,
  );

  assert.equal(
    summary.successMessage,
    "Swagger refreshed: 88 endpoints from 4 definitions.",
  );
  assert.equal(
    summary.warningMessage,
    "Swagger refresh partial: 1 definition failed to import.",
  );
});
