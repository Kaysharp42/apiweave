 ...
 */
function collectNetworkRequests(page: Page): Array<{ url: string; method: string; status: number }> {
...
test.describe('Scoped-Only Boot — Task 14', () => {
  test('authenticated root redirects to personal workspace', async ({ page }) => {
  ...
  test('workflow run triggers scoped API', async ({ page }) => {
    ...
    await navigateAndWait(page, `/personal/workflows/${MOCK_WORKFLOW.workflowId}`);
  ...
  test('environment selector loads scoped environments', async ({ page }) => {
    ...
    const envSelector = page.locator('[aria-label*="environment" i], [data-testid*="environment" i]');
    if (await envSelector.first().isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    const envCalls = networkLog.filter((req) => req.url.includes('/environments'));
    const unscopedEnvCalls = envCalls.filter(
      (req) => !req.url.includes(MOCK_PERSONAL_WORKSPACE.workspaceId) && !req.url.includes('/api/auth/'),
    );
    expect(
      unscopedEnvCalls,
