...
- **Handles:** 8px neutral sockets at rest — a resting handle reads as a socket, not a button. Lit to ink-teal at 11px with a soft accent halo on node hover, while connecting, or on keyboard focus, and to the edge's state color when a live edge is attached. The visual size is never the hit area; a transparent 20px box is — but that box is a **pseudo-element**, never the handle itself.
- **An edge plugs into its socket, flush.** ReactFlow attaches an edge to the handle element's outer face, not its centre, so the handle element is the 8px socket and it sits wholly inside the node: the edge meets the node border and the socket's rim meets the edge. Grow that element into the hit area and the attachment moves out with it — every edge on the canvas then floats in open space, touching neither node. The socket stays visible once connected: it is what makes the join read as a join.
