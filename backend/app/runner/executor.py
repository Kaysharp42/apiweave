...
class WorkflowExecutor:
    ...
    def __init__(self, run_id: str, workflow_id: str, runtime_secrets: dict[str, str] = None):
        ...
        self.runtime_secrets = (
            runtime_secrets or {}
        )  # Secret values provided at run time (never persisted)
        self.results = {}
    ...
    async def execute(self):
        ...
        try:
            ...
            try:
                ...
                print("   Run completed, but status update failed")
            ...
            if "document too large" not in str(e).lower():
                ...
                print("   All results are safely stored in node_results collection")
            ...
            try:
                if var_path.startswith("secrets."):
                    ...
                    if value is not None:
                        self.logger.debug(
                            f"✓ Substituted env variable: {{{{env.{'.'.join(path_parts)}}}}} -> {value}"
                        )
                        return str(value)
                    else:
    ...
    async def _execute_merge(self, node: dict, edges: list, db) -> dict[str, Any]:
        ...
        if merge_strategy == "all":
            ...
            if not completed_predecessors:
                ...
                print("⚠️  [ANY/FIRST strategy] No successful predecessors ready, waiting...")
            ...
            if missing_predecessors:
                ...
                print("⏳ [CONDITIONAL strategy] Waiting for all branches to evaluate conditions")
    ...
    async def _update_node_status(self, db, node_id: str, status: str, result: Any):
        ...
        try:
            ...
            print("   Full result is still available in node_results collection")
