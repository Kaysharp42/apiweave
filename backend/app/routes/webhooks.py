...
from app.models import Run, Webhook, WebhookCreate, WebhookLog, WebhookUpdate
from app.repositories import CollectionRepository, WebhookRepository, WorkflowRepository
...
async def verify_webhook_signature(webhook: Webhook, payload: bytes, signature: str | None) -> bool:
    """Verify HMAC-SHA256 signature for webhook"""
    if not signature or not webhook.hmacSecret:
        return False

    expected_signature = hmac_lib.new(
        webhook.hmacSecret.encode(), payload, hashlib.sha256
    ).hexdigest()
...
async def execute_workflow_webhook(
    ...
    await WebhookRepository.update(
        webhook_id, {"usageCount": webhook.usageCount, "lastUsed": webhook.lastUsed}
    )
