...
from app.repositories import CollectionRepository, EnvironmentRepository, WorkflowRepository
...
async def get_collection(collection_id: str):
    ...
    if not collection:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Collection {collection_id} not found"
        )
...
async def export_collection(collection_id: str, include_environment: bool = Query(True)):
    ...
    for wf in workflows_list:
        ...
        sanitized_vars = sanitize_secrets_in_dict(
            wf_dict.get("variables", {}), wf_secret_refs, "workflows.variables"
        )
...
async def import_collection_dry_run(request: CollectionImportDryRunRequest):
    ...
    if secret_count > 0:
        warnings.append(
            f"Bundle contains {secret_count} secret placeholders - re-enter sensitive values after import"
        )
...
@router.post("/import")
async def import_collection(request: CollectionImportRequest):
    ...
    return {
        "collectionId": collection_id,
        "collectionName": bundle["collection"].get("name", "Imported Collection")
        if createNewCollection
        else collection_id,
...
async def assign_workflow_to_collection(collection_id: str, workflow_id: str):
