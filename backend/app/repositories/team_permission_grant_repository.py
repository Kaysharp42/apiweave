...
class TeamPermissionGrantRepository:
    ...
    async def get_by_id(grant_id: str) -> TeamPermissionGrant | None:
        return await TeamPermissionGrant.find_one(TeamPermissionGrant.grantId == grant_id)
    ...
    async def list_by_team(team_id: str) -> list[TeamPermissionGrant]:
        return await TeamPermissionGrant.find(TeamPermissionGrant.teamId == team_id).to_list()
    ...
    async def list_by_team_ids(team_ids: list[str]) -> list[TeamPermissionGrant]:
        ...
        return await TeamPermissionGrant.find(In(TeamPermissionGrant.teamId, team_ids)).to_list()
