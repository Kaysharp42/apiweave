...
class CollectionRunRepository:
    ...
    @staticmethod
    async def get_by_id(collection_run_id: str) -> CollectionRun | None:
        """
    ...
    async def get_by_collection(
        collection_id: str, skip: int = 0, limit: int = 50
    ) -> list[CollectionRun]:
        """
        ...
        """
        return (
            await CollectionRun.find(CollectionRun.collectionId == collection_id)
            .sort("-startTime")
            .skip(skip)
            .limit(limit)
            .to_list()
    ...
    async def get_by_webhook(
        webhook_id: str, skip: int = 0, limit: int = 50
    ) -> list[CollectionRun]:
        """
        ...
        """
        return (
            await CollectionRun.find(CollectionRun.webhookId == webhook_id)
            .sort("-startTime")
            .skip(skip)
            .limit(limit)
            .to_list()
    ...
    @staticmethod
    async def update(collection_run_id: str, update_data: dict) -> CollectionRun | None:
        """
    ...
    @staticmethod
    async def update_fields(collection_run_id: str, **fields) -> CollectionRun | None:
        """
    ...
    async def add_workflow_result(
        collection_run_id: str, workflow_result: dict
    ) -> CollectionRun | None:
        """
    ...
    @staticmethod
    async def complete(
        collection_run_id: str, status: str, end_time: datetime, duration: int
    ) -> CollectionRun | None:
        """
        ...
        return await CollectionRunRepository.update_fields(
            collection_run_id, status=status, endTime=end_time, duration=duration
        )
    ...
    async def count_by_collection(collection_id: str) -> int:
        ...
        """
        return await CollectionRun.find(CollectionRun.collectionId == collection_id).count()

    @staticmethod
    async def get_latest_by_collection(collection_id: str) -> CollectionRun | None:
        """
