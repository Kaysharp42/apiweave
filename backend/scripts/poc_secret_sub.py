...
def main():
    ...
    print(
        f"[+] environment: HTTP {code} "
        f"id={env.get('environmentId') if isinstance(env, dict) else env}"
    )
        ...
        if secret_in_url_blocked or log_has_block or result_status == "error":
            ...
            results.append(
                (
                    ...
                    False,
                    f"Expected error about secret-in-URL but got "
                    f"status={result_status}, error={result_error[:120]}",
                )
            )
        ...
        if not secret_in_url_blocked and not log_has_block:
            ...
            results.append(
                (
                    "Secret substitution works (vulnerable)",
                    False,
                    f"Expected substitution to proceed but got blocking "
                    f"error: {result_error[:120]}",
                )
            )
