...
async def main() -> None:
    ...
    for email in TEST_EMAILS:
        ...
        deleted = (
            await UserRepository.delete_by_id(u.userId)
            if hasattr(UserRepository, "delete_by_id")
            else None
        )
        print(
            f"[+] Removed test user {email}: "
            f"sessions_revoked={len(sessions or [])} deleted={deleted}"
        )
    await close_db()
