...
def test_csrf_documented_in_security_md() -> None:
    ...
    assert SECURITY_MD_PATH.is_file(), (
        f"Expected documentation file at {SECURITY_MD_PATH}"
    )

    content = SECURITY_MD_PATH.read_text(encoding="utf-8")

    assert "CSRF middleware" in content, (
        "docs/SECURITY.md must contain a 'CSRF middleware' section"
    )
    assert "session cookie" in content, (
