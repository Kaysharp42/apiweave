...
The URL must be reachable from the desktop app's main process, not just from your browser. LAN and local services work without additional configuration. See [Environments and Secrets](environments-and-secrets.md#openapiswagger-url) for the field's full context.
...
The imported group is labeled `Swagger: <Environment Name>` so you can tell at a glance which spec the templates came from. The importer refreshes the group automatically when you open a workflow against that environment. Clicking **Refresh** at any time re-fetches the document and regenerates the group.
...
- **If Refresh reports "Failed to fetch Swagger URL"**, confirm the URL starts with `http://` or `https://` and that the desktop app can reach the host (not just your browser). If a Swagger UI URL fails, try the direct spec URL for the same service.
