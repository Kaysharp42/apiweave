...
| App / `settings` | Canvas interaction/tips, appearance where available, application vs workspace settings; point to actual controls | `app/src/pages/AppSettingsPage.tsx`; `app/src/components/layout/sidebar/SettingsContent.tsx`; `app/src/stores/CanvasPrefsStore.ts` |
...
- Audit and design: complete. DeepSeek Flash inspected the docs and code; GPT-6 Astra reviewed the initial design and shell integration. Later phases ran with DeepSeek Flash only.
...
- Phase 3 accepted (2,675 tests passed, 4 skipped; typecheck, lint, app build and NSIS/MSI packaging passed; nine Playwright tutorial flows passed): command-palette Help entry, contextual tutorial links from Variables and Functions tips, a router-based internal destination icon, a nine-test Playwright tutorial flow (library, search, completion across reload, command palette, follow-along, canvas preservation, compact and dark presentations), and a maintainer/user guide at `docs/features/tutorials.md`. Fixed a shared `useElementWidth` defect where StrictMode tore down the ResizeObserver, pinning consumers to a zero measurement; the header wordmark now hides below the small breakpoint so it cannot collide with controls at 375px. Evidence reviewed at 375/768/1440 in light and dark.
- Graph maintenance: `graphify update .` rebuilt the initially absent graph; the Phase 3 update reports 7,768 nodes and 21,341 edges. Tool warnings identify zero-node JSON fixtures and unavailable SQL parsing; they do not block tutorial execution.
- Product issues surfaced by the audit but left out of tutorial scope: the BootGate error/Retry branch is unreachable, the Run menu's resume entries perform a full run, the MCP panel and SSE URL hints overstate secret support, and project import does not remap Call Workflow targets. Lesson text describes runtime truth rather than the misleading hints.
