...
3. Drag an **Assertion** node onto the canvas. Connect the HTTP node's output to the Assertion's input. Configure a rule: source `status`, operator `equals`, expected value `200`.
4. Pick your environment in the canvas toolbar selector (create one on the **Environments** page first if the selector is empty), then click **Run**. Within a second or two, both nodes turn green. Click a node to see its response body and assertion result.
...
- The green check on the Assertion node confirms the status rule passed: the response code equaled `200`, which is what `https://httpbin.org/get` returns.
...
1. Open **Settings → Secrets** from the left navigation rail.
...
- **If the HTTP node turns red with a connection error**, your local network blocked the outbound call, or the runner's SSRF block rejected the target. Try a different public endpoint or check your proxy. Targets on your LAN (e.g. `192.168.x.x`) need the **Settings → Private networks** opt-in.
