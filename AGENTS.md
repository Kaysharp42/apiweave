...
- For day-to-day development, run `npm run dev` from `app/`; it creates a development-mode renderer bundle, builds Electron, and launches the app with the renderer embedded behind `app://local/`. No renderer HTTP server is started. Restart the command after source changes.
...
- **An agent is briefed by APIWeave, not by the user**: a launched agent is told what it is attached to before the user types anything, because otherwise they retype it every session — and an agent that is not told starts by grepping the working directory for a workflow that was never on disk. Two carriers, and the split is not a preference: the MCP server's `instructions` (`core/mcp/guide.ts`) reach *every* agent because they ride in `initialize`, and are therefore static and paid for on every connection; the per-session briefing file (`core/agents/session_briefing.ts`) carries what only a launch knows — the workflow, the workspace, the folder — and reaches only agents whose CLI has somewhere to put standing instructions: a flag (Claude Code's `--append-system-prompt-file`) or a config file named by an environment variable (OpenCode's `OPENCODE_CONFIG`, whose `instructions` array names the briefing by absolute path — relative paths resolve against the *project* directory, and OpenCode deep-merges config layers per server, so the generated file sets `enabled`/`url`/`headers` explicitly to beat a stale global `apiweave` entry; see `core/agents/opencode_config.ts`). A briefing is a *file* named for its session, never text spliced into argv: on Windows a `.cmd` shim runs through `cmd.exe /c`, where the briefing's own newlines and `&` are command syntax. It is scratch like the MCP config beside it — same per-session owner, same delete on the terminal transition, same startup sweep. Never write either into the user's repository: that folder is a git checkout, which is the same reason `.mcp.json` goes to userData.
- **Native addons must be N-API, and are never rebuilt**: `build.npmRebuild` is `false` and nothing rebuilds addons for Electron's ABI. better-sqlite3 and node-pty both ship N-API prebuilds, which load unchanged under Node (vitest) and Electron. A new native dependency that is V8-ABI rather than N-API brings that whole rebuild problem back — electron-builder would rebuild every native dependency it finds, and node-pty answers a rebuild by deleting its own shipped prebuilds and demanding a C++ toolchain. Anything with a native binary or a worker resolved from `__dirname` also belongs in `build.asarUnpack` — an addon left inside `app.asar` fails at runtime, and node-pty's Windows conout worker fails by *hanging for ever* rather than by throwing.
