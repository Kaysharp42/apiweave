...
export const WorkflowEdgeSchema = z
  .object({
    ...
    sourceHandle: z
      ...
      .optional()
      .describe(
        'Which output port of the source node this edge leaves from. On every edge leaving an assertion node, this must be exactly "pass" or "fail" — an assertion routes its result down the matching branch, and an edge with no handle is never followed. This constrains edges that exist; it does not require an edge per handle. An assertion\'s `fail` handle may be left unconnected: the run records the failed assertion and that branch terminates. Wire `fail` only when you want a distinct failure path. Omit (or null) sourceHandle for every other node type, which has a single output.',
      ),
