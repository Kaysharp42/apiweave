...
`app/electron/cloud/CloudSyncProvider` is registered via the service locator
(`app/core/services-locator.ts`) and drives `pull()` and `push()` against
...
`app/core/id.ts` exposes `generateId()` returning a 26-character ULID
...
The repository layer (`app/core/repositories/`) fills these in on `create`.
...
1. `CloudSyncProvider` in `app/electron/cloud/` implements `SyncProvider`
...
2. It is registered via `setSyncProvider(provider)` at bootstrap
   (`app/electron/main.ts`) when a Cloud account is connected; otherwise
...
4. See `app/electron/cloud/README.md` for the transport, the outbox
