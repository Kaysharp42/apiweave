...
export interface McpToolSpec {
  ...
  /**
   * Writes that change graph topology (new/removed nodes, edges, group
   * membership) are laid out once inside the service write path, on the
...
export const MCP_TOOLS: readonly McpToolSpec[] = [
  ...
  tool("workflows", "get", "read", "Read a workflow: view outline for bounded node ids/types/labels plus edge structure (no positions or configs; outline pages default 100 summaries, max 500, with nextNodeCursor), view nodes for full configs of up to 50 named nodes plus incident edges and boundary-node identities, or view full for the whole graph. Partial views are marked partial:true. Credential values are withheld (`<SECRET>`), while `{{secrets.NAME}}` references remain verbatim. Prefer outline, then nodes, over full on a large workflow."),
  ...
  tool("assertions", "validate", "read", "Check assertion rules and return a human-readable preview plus per-rule issues, without changing the workflow. Use for previews, evidence checks against a runId, or explicit user approval. Canonicalizes what it accepts (a `prev` path gains its `response.` prefix, array indexes use [0]), so the returned `rules` are what assertion_apply should receive.", { name: "assertion_validate" }),
   ...
   tool("workflows", "create", "write", "Create a workflow from nodes, edges and variables. Returns a compact revision/count result and one diagnosis; check it before running. A `workflow` node runs another workflow in the same workspace as one step. Read `apiweave://guide/workflow-authoring` first if you have not built a graph here before. New graphs are laid out automatically; pass `layout: false` to keep the positions sent.", { diagnoseAfterWrite: true, autoLayout: true, resultProjection: "workflowWrite" }),
   tool("workflows", "update", "write", "Replace a workflow's graph, variables or metadata. `nodes` and `edges` are REPLACED wholesale, so send the complete lists; use workflows_patch to change a subset. Returns a compact revision/count result and one diagnosis. Topology changes are laid out automatically; config/label-only replacements keep stored positions. Pass `layout: false` to keep the positions sent.", { idempotent: true, diagnoseAfterWrite: true, autoLayout: true, resultProjection: "workflowWrite" }),
   tool("workflows", "patch", "write", "Change part of a workflow with upsertNodes, upsertEdges, removeNodeIds and removeEdgeIds, and merge workflow variables. Insert a node and rewire its connections in ONE patch: include the new node, old edge removals and replacement edges together so the saved branch stays connected. Existing upserted nodes merge recursively; new nodes need type and complete config. Returns bounded touched IDs, revision/counts, and one diagnosis. Pass expectedRevision from the rev you last read to reject stale edits. See apiweave://guide/workflow-authoring for a complete insertion example. Topology changes are laid out automatically on the saved revision; config/label-only patches keep every position. Pass layout: false to keep positions exactly as stored/sent.", { idempotent: true, diagnoseAfterWrite: true, autoLayout: true, resultProjection: "workflowWrite" }),
   tool("workflows", "layout", "write", "Re-lay-out a workflow's node positions with dagre (a layered left-to-right or top-to-bottom flow) without changing anything else. Returns a compact revision/count result and one diagnosis. Use this on a workflow whose nodes ended up stacked or overlapping — an import, or a graph built before this tool existed.", { idempotent: true, diagnoseAfterWrite: true, resultProjection: "workflowWrite" }),
  tool("workflows", "delete", "write", "Delete a workflow.", { destructive: true, idempotent: true }),
   tool("workflows", "attachToCollection", "write", "Attach a workflow to a project, or detach it with collectionId null — this is the project-membership write. Returns a compact revision/count result and one diagnosis.", { idempotent: true, diagnoseAfterWrite: true, resultProjection: "workflowWrite" }),
   tool("workflows", "setEnvironment", "write", "Set or clear the selected environment for a workflow. Returns a compact revision/count result and one diagnosis.", { idempotent: true, diagnoseAfterWrite: true, resultProjection: "workflowWrite" }),
   tool("workflows", "moveToWorkspace", "write", "Move a workflow into another workspace, optionally attaching it to a project there (`targetCollectionId`, or null to leave it unassigned). Its selected environment is cleared and any `workflow` node targeting a workflow left behind loses that target — a workspace is the scope those references resolve in, so they cannot follow. Returns a compact revision/count result and one diagnosis. Confirm with the user before calling: nothing here is recoverable by moving the workflow back.", { destructive: true, diagnoseAfterWrite: true, resultProjection: "workflowWrite" }),
...
// the latest failure adds status failed. Single-run reads stay granular.
  ...
  tool("runs", "get", "read", "Get a metadata-only run snapshot with per-node status. Its `runRev` is only this run record's revision, never a workflow revision; use workflows_get's `rev` for expectedRevision.", { resultProjection: "run" }),
