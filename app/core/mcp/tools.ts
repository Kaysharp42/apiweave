...
export const MCP_TOOLS: readonly McpToolSpec[] = [
  ...
  tool("workflows", "patch", "write", "Change part of a workflow without resending the whole graph: upsert or remove nodes and edges by id, and merge workflow variables. An existing upserted node is merged recursively — omit config fields and position to preserve them; a new node must include its type and complete config. Returns the updated workflow with a `diagnosis` report. Pass `expectedRevision` to make it a compare-and-swap against the revision you last read. The WHOLE graph (not just the touched nodes) is re-laid-out with dagre before saving; pass `layout: false` to keep positions exactly as stored/sent.", { idempotent: true, diagnoseAfterWrite: true, autoLayout: true }),
