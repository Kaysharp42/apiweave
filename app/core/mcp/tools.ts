...
export const MCP_TOOLS: readonly McpToolSpec[] = [
  ...
  tool("workflows", "get", "read", "Get a workflow graph, or just selected nodes with `nodeIds` on a large workflow. A filtered read keeps workflow metadata and returns only the requested nodes plus edges directly between them. Credential values are withheld (`<SECRET>`), while `{{secrets.NAME}}` references remain verbatim. Reading an existing workflow is the fastest way to learn this workspace's conventions before authoring a new one."),
  ...
  tool("workflows", "patch", "write", "Change part of a workflow with upsertNodes, upsertEdges, removeNodeIds and removeEdgeIds, and merge workflow variables. Insert a node and rewire its connections in ONE patch: include the new node, old edge removals and replacement edges together so the saved branch stays connected. An existing upserted node is merged recursively — omit config fields and position to preserve them; a new node must include its type and complete config. Returns a compact summary by default: check touchedNodeIds, touchedEdgeIds and diagnosis before running. Diagnosis is after saving; success alone does not mean the graph is runnable. Pass expectedRevision from the rev you last read to reject stale edits. See apiweave://guide/workflow-authoring for a complete insertion example. The WHOLE graph is re-laid-out before saving; pass layout: false to keep positions exactly as stored/sent.", { idempotent: true, diagnoseAfterWrite: true, autoLayout: true }),
