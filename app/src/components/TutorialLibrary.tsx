...
/** Title, completion progress, the optional back-to-workspace button, and the
 * search field. */
function LibraryHeader({
  ...
  headingRef: TutorialLibraryProps["headingRef"];
  progress: TutorialLibraryProps["progress"];
...
/** The resume-or-start card, with copy that depends on how far the reader has
 * gotten. Hidden while searching. */
function LibraryResumeCard({
  resume,
  onSelectLesson,
}: {
  resume: NonNullable<TutorialLibraryProps["resume"]>;
  onSelectLesson: (lessonId: string) => void;
}) {
  const title = resume.allComplete
    ? "You've finished every lesson"
    : resume.started
      ? "Continue where you left off"
      : "Start here";
  const description = resume.allComplete
    ? "Revisit any lesson whenever you like."
    : "Pick up the lesson and follow along.";
  const buttonLabel = resume.allComplete
    ? "Revisit lesson"
    : resume.started
      ? "Continue"
      : "Start lesson";
...
/** Search results (or the no-matches empty state) when searching; the
...
function LibraryResults({
  ...
  isSearching: boolean;
  query: string;
  results: ReturnType<typeof searchTutorialLessons>;
  activeLessonId: string | null;
  completedLessonIds: ReadonlySet<string>;
  onSelectLesson: (lessonId: string) => void;
  onQueryChange: (value: string) => void;
  chapters: TutorialLibraryProps["chapters"];
  ...
  return (
    <div>
      <p className="px-2.5 pb-2 text-xs text-text-secondary dark:text-text-secondary-dark">
        {results.length} {results.length === 1 ? "lesson" : "lessons"} match “
        {query.trim()}”.
      </p>
      <ul className="space-y-0.5">
        {results.map((result) => (
    ...
    <div className="flex h-full min-h-0 min-w-0 flex-col">
      ...
      <div className="min-h-0 min-w-0 flex-1 overflow-y-auto pb-4">
        {resume !== null && !isSearching && (
          <LibraryResumeCard resume={resume} onSelectLesson={onSelectLesson} />
        )}
