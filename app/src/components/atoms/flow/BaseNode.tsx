...
export function BaseNode({
  ...
  > = {
    ...
    running: {
      ...
      badge:
        "bg-status-running/15 text-status-running dark:text-[var(--aw-status-running)]",
      icon: (
        <Loader2
    ...
    success: {
      ...
      badge:
        "bg-status-success/15 text-status-success dark:text-[var(--aw-status-success)]",
    ...
    error: {
      ...
      dot: "bg-status-error",
      badge:
        "bg-status-error/15 text-status-error dark:text-[var(--aw-status-error)]",
    ...
    warning: {
      ...
      badge:
        "bg-status-warning/15 text-status-warning dark:text-[var(--aw-status-warning)]",
      ...
      >
        {title && (
          ...
          >
            {icon && (
              <span
                className={`flex-shrink-0 flex items-center justify-center ${headerTextClass || "text-text-secondary dark:text-text-secondary-dark"}`}
              >
                {icon}
              </span>
            )}
            ...
            {config.badge && statusBadgeText && (
              <span
                className={`inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-sm font-mono ${config.badge}`}
              >
                {config.icon}
                {statusBadgeText}
              </span>
