...
test("getAccountDisplayName prefers display name and falls back to verified email", () => {
  ...
  assert.equal(
    getAccountDisplayName({ ...baseUser, display_name: "   " }),
    "jane.doe",
  );
  assert.equal(
    getAccountDisplayName({
...
test("getAccountInitials derives a stable two-letter fallback", () => {
  ...
  assert.equal(
    getAccountInitials(
      { ...baseUser, verified_email: "", userId: "user-789" },
      "",
    ),
    "U",
  );
...
test("getRoleSummary shows role-specific account summary", () => {
  assert.equal(
    getRoleSummary({ ...baseUser, roles: ["admin"] }),
    "Admin · full access",
  );
  assert.equal(
    getRoleSummary({ ...baseUser, roles: ["editor"] }),
    "Editor · workflow author",
  );
  assert.equal(
    getRoleSummary({ ...baseUser, roles: ["viewer"] }),
    "Viewer · read only",
  );
  assert.equal(
    getRoleSummary({ ...baseUser, roles: ["support", "auditor"] }),
    "Support · Auditor",
  );
