  ...
  return (
    <header className="sticky top-0 z-50 bg-surface/90 dark:bg-surface-dark/90 backdrop-blur-sm border-b border-border dark:border-border-dark">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8 h-14">
        <Link
          to="/"
          className="font-display font-extrabold tracking-tight text-text-primary dark:text-text-primary-dark text-lg"
        >
          APIWeave
        </Link>
        <nav className="flex items-center gap-2 sm:gap-4">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="hidden sm:inline-flex items-center gap-1.5 text-sm text-text-secondary dark:text-text-secondary-dark hover:text-text-primary dark:hover:text-text-primary-dark transition-colors"
          >
          ...
          <a
            href="#docs"
            className="hidden sm:inline-flex text-sm text-text-secondary dark:text-text-secondary-dark hover:text-text-primary dark:hover:text-text-primary-dark transition-colors"
          >
            Docs
          </a>
          <button
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="flex flex-col gap-6">
            ...
            <p className="text-base md:text-lg text-text-secondary dark:text-text-secondary-dark max-w-lg leading-relaxed">
              Assemble test flows on a canvas from drag-and-drop nodes. Chain
              requests with extracted variables. Run against scoped
              environments. Self-hosted, open-source.
            </p>
            ...
            <div className="flex items-center gap-4 pt-4 text-xs font-mono text-text-muted dark:text-text-muted-dark">
              ...
              <span className="flex items-center gap-1.5">
                <Server className="w-3.5 h-3.5" /> Self-hostable
              </span>
              <span className="flex items-center gap-1.5">
                <Lock className="w-3.5 h-3.5" /> Write-only secrets
              </span>
            </div>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 lg:gap-12">
          {STEPS.map((step) => (
            <div key={step.n} className="flex flex-col gap-4 min-w-0">
              ...
              <pre className="mt-auto font-mono text-[11px] leading-relaxed text-text-secondary dark:text-text-secondary-dark bg-surface-overlay dark:bg-surface-dark-overlay border border-border dark:border-border-dark rounded-sm p-3 overflow-x-auto max-w-full">
                <code>{step.code}</code>
              </pre>
            </div>
          ))}
...
const FEATURES = [
  {
    Icon: Globe,
    title: "HTTP Request nodes",
    body: "All methods, headers, body, extractors, and file uploads. JSONPath variable extraction from responses.",
  },
  {
    Icon: CheckCircle2,
    title: "Assertions",
    body: "10 operators, 5 sources, nested paths with array indexing. Pass/fail tracked per node.",
  },
  {
    Icon: Network,
    title: "Branches & merges",
    body: "Parallel branches run concurrently. Merge nodes sync them with an asyncio lock.",
  },
  {
    Icon: KeyRound,
    title: "Scoped secrets",
    body: "Libsodium sealed-box ingress, write-only through every layer. User, org, workspace, and environment scopes.",
  },
  {
    Icon: Webhook,
    title: "Webhooks",
    body: "Token + HMAC auth, idempotency, rate limiting. Trigger runs from CI/CD with scoped service tokens.",
  },
  {
    Icon: Boxes,
    title: "MCP server",
  ...
  {
    Icon: FileCode2,
    title: "OpenAPI / HAR / cURL import",
    body: "Turn a spec into reusable request nodes. Import a cURL command straight onto the canvas.",
  },
  {
    Icon: Database,
    title: "Collections & projects",
    body: "Group workflows into ordered runs. Export and import .awecollection bundles — secrets sanitized.",
  },
] as const;
    ...
    <section className="border-b border-border dark:border-border-dark">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
        <div className="mb-12 md:mb-16">
          ...
          <p className="mt-3 text-text-secondary dark:text-text-secondary-dark max-w-2xl">
            A complete, self-hostable workspace — not a thin wrapper around
            curl.
          </p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-px bg-border dark:bg-border-dark border border-border dark:border-border-dark rounded-sm overflow-hidden">
          {FEATURES.map((f) => (
            <div
              key={f.title}
              className="bg-surface-raised dark:bg-surface-dark-raised p-6 flex flex-col gap-3"
            >
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
        <div className="mb-12 md:mb-16 max-w-3xl">
          <div className="flex items-center gap-2 mb-4">
            <span className="font-mono text-xs text-primary dark:text-primary-light border border-primary/30 dark:border-primary-light/30 rounded-sm px-2 py-0.5">
              MCP
            </span>
            <span className="font-mono text-xs text-text-muted dark:text-text-muted-dark">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-border dark:bg-border-dark border border-border dark:border-border-dark rounded-sm overflow-hidden">
                {MCP_DOMAINS.map((d) => (
                  <div
                    key={d.name}
                    className="bg-surface-raised dark:bg-surface-dark-raised p-3 flex flex-col gap-1"
                  >
                    <span className="font-mono text-lg font-bold text-text-primary dark:text-text-primary-dark leading-none">
                      {d.count}
                    </span>
          ...
          <div className="bg-surface-raised dark:bg-surface-dark-raised border border-border dark:border-border-dark rounded-sm overflow-hidden">
            ...
            <pre className="p-4 sm:p-6 font-mono text-xs leading-relaxed text-text-primary dark:text-text-primary-dark overflow-x-auto max-w-full">
              <code>{`User: "Test the /login endpoint
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="flex flex-col gap-6">
            ...
            <p className="text-text-secondary dark:text-text-secondary-dark leading-relaxed">
              Python 3.13+, Node.js 20+, MongoDB 7+. The first sign-in becomes
              your instance owner. No telemetry, no cloud dependency.
            </p>
          ...
          <div className="bg-surface-raised dark:bg-surface-dark-raised border border-border dark:border-border-dark rounded-sm overflow-hidden">
            ...
            <pre className="p-4 sm:p-6 font-mono text-xs sm:text-sm leading-relaxed text-text-primary dark:text-text-primary-dark overflow-x-auto">
              <code>{`# Clone
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="flex flex-col gap-1">
            <span className="font-display font-extrabold tracking-tight text-text-primary dark:text-text-primary-dark">
              APIWeave
            </span>
            <span className="font-mono text-xs text-text-muted dark:text-text-muted-dark">
              Visual API test workflows. Open-source, MIT.
            </span>
          </div>
        ...
        <div className="mt-8 pt-6 border-t border-border dark:border-border-dark flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <span className="font-mono text-[10px] text-text-muted dark:text-text-muted-dark">
            © {new Date().getFullYear()} APIWeave. MIT licensed.
          </span>
          <span className="font-mono text-[10px] text-text-muted dark:text-text-muted-dark flex items-center gap-1.5">
            <GitBranch className="w-3 h-3" /> Self-hosted. No telemetry.
          </span>
        </div>
      </div>
    </footer>
