...
const asKeyboardEvent = (event: unknown): KeyboardEvent =>
...
const createSelectionDoc = ({
  text = "",
  isCollapsed = false,
  rangeCount = 1,
}: SelectionDocOptions = {}): Document =>
  asDocument({
    getSelection: () => ({
      rangeCount,
      isCollapsed,
...
test("isEditableKeyboardTarget detects input and textarea", () => {
  assert.equal(
    isEditableKeyboardTarget(asEventTarget({ tagName: "input" })),
    true,
  );
  assert.equal(
    isEditableKeyboardTarget(asEventTarget({ tagName: "textarea" })),
    true,
  );
});
...
test("isEditableKeyboardTarget detects contenteditable and rich editor wrappers", () => {
  assert.equal(
    isEditableKeyboardTarget(asEventTarget({ isContentEditable: true })),
    true,
  );

  const richTarget = {
    closest: (selector: string) =>
      selector.includes(".monaco-editor") ? {} : null,
  };
...
test("hasSelectedText returns true only for non-empty user selections", () => {
  assert.equal(
    hasSelectedText(createSelectionDoc({ text: "abc", isCollapsed: false })),
    true,
  );
  assert.equal(
    hasSelectedText(createSelectionDoc({ text: "   ", isCollapsed: false })),
    false,
  );
  assert.equal(
    hasSelectedText(createSelectionDoc({ text: "abc", isCollapsed: true })),
    false,
  );
  assert.equal(
    hasSelectedText(createSelectionDoc({ text: "abc", rangeCount: 0 })),
    false,
  );
});
