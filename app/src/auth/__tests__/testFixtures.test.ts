...
test("mockAdmin: /api/auth/me returns admin user with all permissions", async () => {
  ...
  try {
    const user = (await authenticatedJson("/api/auth/me")) as ReturnType<
      typeof buildAdminUser
    >;
    assert.equal(user.userId, FIXTURE_ADMIN_USER_ID);
    assert.ok(user.roles.includes("admin"), "Should have admin role");
    assert.ok(
      user.permissions.includes("users:invite"),
      "Admin should have users:invite",
    );
    assert.ok(
      user.permissions.includes("settings:update"),
      "Admin should have settings:update",
    );
    assert.ok(
      user.permissions.includes("workflows:create"),
      "Admin should have workflows:create",
    );
...
test("mockEditor: /api/auth/me returns editor user", async () => {
  ...
  try {
    const user = (await authenticatedJson("/api/auth/me")) as ReturnType<
      typeof buildEditorUser
    >;
    assert.equal(user.userId, FIXTURE_EDITOR_USER_ID);
    assert.ok(user.roles.includes("editor"), "Should have editor role");
    assert.ok(
      user.permissions.includes("workflows:create"),
      "Editor should have workflows:create",
    );
    assert.ok(
      user.permissions.includes("collections:create"),
      "Editor should have collections:create",
    );
    assert.ok(
      !user.permissions.includes("users:invite"),
      "Editor must NOT have users:invite",
    );
    assert.ok(
      !user.permissions.includes("settings:update"),
      "Editor must NOT have settings:update",
    );
    assert.equal(user.is_setup_complete, true);
  } finally {
    restore();
...
test("mockViewer: /api/auth/me returns viewer user (read-only)", async () => {
  ...
  try {
    const user = (await authenticatedJson("/api/auth/me")) as ReturnType<
      typeof buildViewerUser
    >;
    assert.equal(user.userId, FIXTURE_VIEWER_USER_ID);
    assert.ok(user.roles.includes("viewer"), "Should have viewer role");
    assert.ok(
      user.permissions.includes("workflows:read"),
      "Viewer should have workflows:read",
    );
    assert.ok(
      !user.permissions.includes("workflows:create"),
      "Viewer must NOT have workflows:create",
    );
    assert.ok(
      !user.permissions.includes("workflows:delete"),
      "Viewer must NOT have workflows:delete",
    );
    assert.ok(
      !user.permissions.includes("users:invite"),
      "Viewer must NOT have users:invite",
    );
    assert.equal(user.is_setup_complete, true);
  } finally {
    restore();
...
test("restore: fetch is restored after mock", async () => {
  ...
  assert.equal(
    globalThis.fetch,
    originalFetch,
    "fetch should be restored to original",
  );
});
...
test("mockDeploymentMode: /api/auth/mode returns single_user", async () => {
  ...
  try {
    const body = (await authenticatedJson("/api/auth/mode")) as {
      mode: string;
    };
