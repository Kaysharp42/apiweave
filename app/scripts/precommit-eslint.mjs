...
//
// This script does both: sets the env var and re-execs ESLint with cwd=app/.
import { spawnSync } from "node:child_process";
import { dirname, resolve, isAbsolute, relative } from "node:path";
import { fileURLToPath } from "node:url";

const appDir = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const eslintBin = resolve(appDir, "node_modules/eslint/bin/eslint.js");

const args = process.argv
  .slice(2)
  .filter((arg) => {
    const abs = isAbsolute(arg) ? arg : resolve(arg);
    // Pre-commit may pass paths outside app/. Skip them silently.
    return abs.startsWith(appDir);
  })
  .map((arg) => {
    const abs = isAbsolute(arg) ? arg : resolve(arg);
    return relative(appDir, abs).replaceAll("\\", "/");
  });
