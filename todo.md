...
- [ ] Create new directory structure:
  ...
  ```
- [ ] Create `atoms/Button.jsx` — DaisyUI `btn` with intent variants (primary/secondary/success/error/warning/ghost), sizes (xs/sm/md/lg), loading state
- [ ] Create `atoms/Badge.jsx` — DaisyUI `badge` with color variants
- [ ] Create `atoms/Input.jsx` — DaisyUI `input` with label, error state, helper text
- [ ] Create `atoms/TextArea.jsx` — DaisyUI `textarea` with auto-resize
- [ ] Create `atoms/IconButton.jsx` — Icon-only button with tooltip (replaces scattered icon+onClick patterns)
- [ ] Create `atoms/Divider.jsx` — Horizontal/vertical divider component
- [ ] Create `atoms/Tooltip.jsx` — Wrapper around Tippy.js with consistent styling
- [ ] Create `atoms/Toggle.jsx` — DaisyUI `toggle` for boolean settings (replaces custom toggles)
- [ ] Create `atoms/Spinner.jsx` — DaisyUI `loading` with size variants
- [ ] Create `atoms/flow/BaseNode.jsx` — Shared node shell (border, title bar with icon+label+status, content area, handles). Inspired by FlowTest's `FlowNode.js`
- [ ] Create `atoms/flow/NodeHandle.jsx` — Styled ReactFlow handle (rectangular like FlowTest, color-coded)
- [ ] Create `molecules/ConfirmDialog.jsx` — Replace all `window.confirm()` / `window.alert()` with styled DaisyUI modal
- [ ] Create `molecules/EmptyState.jsx` — Reusable empty state with icon, title, description, optional CTA button (inspired by FlowTest's `EmptyWorkSpaceContent`)
- [ ] Create `molecules/KeyValueEditor.jsx` — Reusable key-value pair table (used by headers, env vars, extractors)
- [ ] Create `molecules/StatusBadge.jsx` — Execution status indicator (running/success/fail/pending)
- [ ] Export all atoms from `atoms/index.js` barrel file
- [ ] Export all molecules from `molecules/index.js` barrel file
...
- [ ] Audit all icon imports across all 49+ JSX files (grep for `react-icons`, `lucide-react`, `@heroicons`)
- [ ] Create an icon mapping file `src/utils/icons.js` that re-exports canonical icons from `lucide-react`:
  ```js
  ...
  ```
- [ ] Replace all `react-icons` imports (MdXxx, BsXxx, BiXxx, HiXxx, AiXxx) with lucide equivalents across all components
- [ ] Replace all `@heroicons/react` imports with lucide equivalents
- [ ] Remove `react-icons` and `@heroicons/react` from `package.json`
- [ ] Install `react-hot-toast` or `sonner` (modern toast library):
  - `npm install sonner` (preferred — lightweight, beautiful defaults, dark mode support)
- [ ] Create `atoms/Toast.jsx` wrapper around sonner with APIWeave styling
- [ ] Replace all custom `Toaster.jsx` event dispatches (`window.dispatchEvent(new CustomEvent('toast', ...))`) with direct `toast.success()` / `toast.error()` calls
- [ ] Remove old `Toaster.jsx` component
- [ ] Verify all icons render correctly in both light and dark mode
...
- [ ] **MainHeader.jsx** redesign:
  - [ ] Left: Logo + "APIWeave" wordmark (Montserrat display font)
  - [ ] Center: (empty, or future breadcrumb)
  - [ ] Right: Environment dropdown (DaisyUI select), Auto-save toggle (DaisyUI toggle), Dark mode toggle (DaisyUI swap icon), User avatar placeholder
  - [ ] Use DaisyUI `navbar` component as base
  - [ ] Height: consistent 48px, borders using design tokens
- [ ] **MainFooter.jsx** redesign:
  - [ ] Left: Version tag (DaisyUI badge)
  - [ ] Center: Status indicator (Ready / Running / Error)
  - [ ] Right: Collapse sidebar toggle, GitHub link icon
  - [ ] Use `bg-surface` classes, 32px height
- [ ] **AppNavBar.jsx** redesign:
  - [ ] Match FlowTest: vertical icon rail with cyan-900 active indicator bar (4px left border)
  - [ ] Collapse/expand with smooth Headless UI Transition (labels slide in/out)
  - [ ] Items: Workflows (default), Collections, Webhooks, Settings (disabled badge "Soon")
  - [ ] Collapsed width: 56px (icon-only) with tooltips
  - [ ] Expanded width: 180px (icon + label)
  - [ ] Use design token colors, lucide icons
- [ ] **MainLayout.jsx** — update Allotment constraints:
  - [ ] Left pane: min 56px (collapsed), preferred 450px, max 600px
  - [ ] Separator visible/disabled state based on collapse
  - [ ] Smooth transition when collapsing
- [ ] **Home.jsx** — add conditional layout: show `WithoutSidebar` variant when no workflows exist (like FlowTest's pattern)
- [ ] Add `HorizontalDivider` atom if not already present (consistent 1px border)
...
- [ ] **SidebarHeader** redesign:
  - [ ] Breadcrumb-style header: "My Workspace > Workflows" (dynamic based on nav selection)
  - [ ] Action buttons (New + Import) as DaisyUI `btn-ghost btn-sm` with lucide icons
  - [ ] Use `molecules/SearchInput.jsx` for sidebar search/filter
- [ ] **Workflows view** redesign:
  - [ ] DaisyUI `menu` tree structure (like FlowTest's `Collection.js`)
  - [ ] Each workflow item: file icon, name, node count badge, collection badge, env badge
  - [ ] Hover state: subtle background + export/delete action icons appear
  - [ ] Selection state: `bg-primary/10` with left border accent
  - [ ] Add loading skeleton states (DaisyUI `skeleton`) during fetch
  - [ ] Virtual scrolling for large lists (or intersection observer for pagination)
- [ ] **Collections view** redesign:
  - [ ] Collapsible tree: DaisyUI `collapse` or `menu` with nested items
  - [ ] Collection header: folder icon, name, workflow count badge, chevron
  - [ ] Nested workflow items indented with vertical guide lines (CSS `before:` pseudo like FlowTest)
  - [ ] Context menu on right-click (or 3-dot menu) for CRUD actions
- [ ] **Webhooks view**: Keep WebhookManager inline but restyle with DaisyUI components
- [ ] **Settings view**: Design placeholder with actual setting categories (General, Editor, Theme, About)
- [ ] Add **empty states** using `molecules/EmptyState.jsx`:
  - [ ] Workflows: illustration + "No workflows yet" + "Create your first workflow" CTA button
  - [ ] Collections: illustration + "No collections yet" + CTA
- [ ] Replace `window.dispatchEvent('workflowsNeedRefresh')` with Zustand store action (partial event migration)
