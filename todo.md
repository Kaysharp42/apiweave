...
- [ ] Not done - Reproduce with the exact affected workflow and record the timeline (open workflow, wait, edit, save/autosave, refresh).
- [ ] Not done - Capture API payloads for `GET /api/workflows/{id}`, list endpoints (`/api/workflows`, `/api/workflows/unattached`), and every `PUT /api/workflows/{id}` fired by the editor.
- [ ] Not done - Confirm whether the database already contains only the start node or whether the collapse happens in the frontend before save.
...
- [ ] Not done - Commit Phase 1 with message format: `test(regression): reproduce workflow graph collapse issue`.
...
- [ ] Not done - Commit Phase 2 with message format: `test(workspace): lock expected workflow hydration behavior`.
...
- [x] Done - Run phase tests and checks.
- [ ] Not done - Commit Phase 3 with message format: `fix(workspace): hydrate workflow editor from canonical workflow payload`.
...
- [x] Done - Run phase tests and checks.
- [ ] Not done - Commit Phase 4 with message format: `fix(editor): block destructive autosave on incomplete graph state`.

## Phase 5 - Recovery and Verification of Affected Data
- [ ] Not done - Verify whether the affected 14-node workflow can be restored from history/export/backups (or reconstruct from run artifacts if available).
- [ ] Not done - Validate restored workflow integrity (node count, edge count, variable mappings, execution path).
- [ ] Not done - Add a one-time verification script/checklist for other workflows that may have been overwritten similarly.
- [ ] Not done - Write tests for any recovery script/validator logic introduced.
- [ ] Not done - Run phase tests and checks.
- [ ] Not done - Commit Phase 5 with message format: `chore(recovery): validate and restore impacted workflows`.

## Phase 6 - Final QA and Rollout
- [ ] Not done - Run full smoke QA: open existing large workflows, edit nodes, autosave, reload, switch tabs, and verify graph persistence.
- [ ] Not done - Verify no regression in JSON editor import/apply and workflow create flow.
- [ ] Not done - Document root cause, fix details, and operator runbook in `progress/learnings.md`.
- [ ] Not done - Write final integration/regression test additions (if any remaining gap exists).
- [ ] Not done - Run final test suite/build checks.
- [ ] Not done - Commit Phase 6 with message format: `docs+qa: finalize workflow editor persistence regression fix`.

---

## Phase Exit Rule (applies to every phase)
- [ ] Not done - Every phase must include at least one test addition/update.
- [ ] Not done - Every phase must end with: tests passing -> commit created -> short note of what changed and why.
